...
#         self.obspos_y = int(self.Ny/2) 
#         self.obspos_x = int(self.Nx/2) 
        self.obspos_y = np.arange( int(self.Ny*0.45) , int(self.Ny*0.55)  )
        #self.obspos_y = int(self.Ny/2) 
        self.obspos_y_2 = int(self.Ny/2) 
        self.obspos_x = int(self.Nx/4) 
        self.obspos_x_2 = int(self.Nx/4*3) 
        ...
        self.dt = self.cfl * np.sqrt( self.mu * self.epsilon ) / np.sqrt( 1.0 / ( self.dx * self.dx ) + 1.0 / ( self.dy * self.dy ) );
        print(self.dt)
        #self.cfl = 0.9
        self.dt = self.cfl * np.sqrt( self.mu * self.epsilon ) * self.dx / np.sqrt(2) * self.n_slid;
        print(self.dt)
    ...
    def updateEzEfficient(self):
        ...
        Ez_const = 1.0 / (1.0 + ((self.dt / 2. * self.sig) / self.eps))
...
#         print(np.shape( self.E_z[1:-1, 1:-1]))
        ...
        self.E_z[1:-1, 1:-1] = self.dt /1.0 / self.eps[1:-1, 1:-1] * ((-((Hx_N) - (Hx_S)) / self.dy \
                             + ((Hy_E) - (Hy_W)) / self.dx) - self.sig[1:-1, 1:-1] *self.E_z[1:-1, 1:-1])+ self.E_z[1:-1, 1:-1];
    ...
    def updateEz(self, x_iter_Ez, y_iter_Ez):
        ...
        #print('Ez old')
        #print(self.E_z[y_iter_Ez, x_iter_Ez])
        self.E_z[y_iter_Ez, x_iter_Ez] = self.dt /1.0 / self.eps[y_iter_Ez, x_iter_Ez] * ((((Hx_S) - (Hx_N)) / self.dy \
                                                        - ((Hy_W) - (Hy_E)) / self.dx) - self.sig[y_iter_Ez, x_iter_Ez] *self.E_z[y_iter_Ez, x_iter_Ez])+ self.E_z[y_iter_Ez, x_iter_Ez];
        #print('Ez new')
        #print(self.E_z[y_iter_Ez, x_iter_Ez])
    ...
    def updateHy(self, x_iter_Hy, y_iter_Hy):
        self.H_y[y_iter_Hy, x_iter_Hy] = self.H_y[y_iter_Hy, x_iter_Hy] + (self.dt / self.mu) \
                                         * ((self.E_z[y_iter_Hy, x_iter_Hy]) - (self.E_z[y_iter_Hy + 1, x_iter_Hy])) / self.dx;
...
#                                          * ((self.E_z[1:,:]) - (self.E_z[:-1,:])) / self.dx;
    ...
    def updateHx(self, x_iter_Hx, y_iter_Hx):
        self.H_x[y_iter_Hx, x_iter_Hx] = self.H_x[y_iter_Hx, x_iter_Hx] - (self.dt / self.mu) \
                                         * ((self.E_z[y_iter_Hx, x_iter_Hx]) - (self.E_z[y_iter_Hx, x_iter_Hx + 1])) / self.dy;
...
#                                          * ((self.E_z[:, 1:]) - (self.E_z[:, :-1])) / self.dy;
    ...
    def setObstacle(self,xMin = 0.25, xMax = 0.75, yMin = 0.25, yMax = 0.75):
        self.obstacle = True
        self.eps[int(self.Ny*yMin):int(self.Ny*yMax),int(self.Nx*xMin):int(self.Nx*xMax)] \
        = self.eps0 * ( self.n_slid * self.n_slid - self.k_slid * self.k_slid );
        self.sig[int(self.Ny*yMin):int(self.Ny*yMax),int(self.Nx*xMin):int(self.Nx*xMax)] \
        = 4.0 * np.pi * np.sqrt( self.eps0 / self.mu0 ) * ( self.n_slid * self.k_slid ) / self.wavelength;
   
    def applyMurBoundaryCondition(self):
       # print('using mur bc')
        self.E_z[0,:] = self.E_z_old[1,:] + self.mur_const*(self.E_z[1,:]-self.E_z[0,:])
        self.E_z[-1,:] = self.E_z_old[-2,:] + self.mur_const*(self.E_z[-2,:]-self.E_z[-1,:])
        self.E_z[:,0] = self.E_z_old[:,1] + self.mur_const*(self.E_z[:,1]-self.E_z[:,0])
        self.E_z[:,-1] = self.E_z_old[:,-2] + self.mur_const*(self.E_z[:,-2]-self.E_z[:,-1])
    ...
    def updateFieldEfficient(self):
        ...
        self.E_z[ self.obspos_y , self.obspos_x ] = ( 1.0 * np.sin( self.omega * t ) )
        #self.E_z[ self.obspos_y , self.obspos_x_2 ] = ( 1.0 * np.sin( self.omega * t ) )
    ...
    def updateField(self):
        ...
        '''
        #gauss = np.exp(-((t-6*self.tau)/self.tau)**2)
        self.E_z[ int(self.Ny/4) , int(self.Nx/4) ] = ( 1.0 * np.sin( self.omega * t ) )
        ...
        if (self.mur):           
            for x_iter_Ez in range(0,self.Nx+1):
                for y_iter_Ez in range(0,self.Ny+1):
                    if (y_iter_Ez == 0):
                        self.E_z[y_iter_Ez,x_iter_Ez] = self.E_z_old[y_iter_Ez+1,x_iter_Ez] + self.mur_const*(self.E_z[y_iter_Ez+1,x_iter_Ez]-self.E_z[y_iter_Ez,x_iter_Ez])
                        
                    if (y_iter_Ez == self.Ny):
                        self.E_z[y_iter_Ez,x_iter_Ez] = self.E_z_old[y_iter_Ez-1,x_iter_Ez] + self.mur_const*(self.E_z[y_iter_Ez-1,x_iter_Ez]-self.E_z[y_iter_Ez,x_iter_Ez])
                        
                    if (x_iter_Ez == 0):
                        self.E_z[y_iter_Ez,x_iter_Ez] = self.E_z_old[y_iter_Ez,x_iter_Ez+1] + self.mur_const*(self.E_z[y_iter_Ez,x_iter_Ez+1]-self.E_z[y_iter_Ez,x_iter_Ez])
