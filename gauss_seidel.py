...
class GaussSeidel(object):
    ...
    def __init__(self, n_x = 100 , n_y = 100 , x_len = 1.0, y_len = 1.0, i_iter = 100):
        '''
        Constructor
        '''
        self.n_x = n_x
        ...
        self.y_len = y_len
        self.i_iter = i_iter
        self.h_x = self.x_len / self.n_x
        self.h_y = self.y_len / self.n_y
   
    def run(self):     
        n_x = self.n_x
        n_y = self.n_y        
        h_x = self.x_len / self.n_x
        h_y = self.y_len / self.n_y
        ...
        for i in range(0,self.i_iter):
            ...
            for y in range(1,n_y):
                for x in range(1,n_x):
                    self.u_h[x + (n_x+1)*y] =  0.25 * (self.u_h[x-1 + (n_x+1)* y] \
